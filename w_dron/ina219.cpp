...
#define INA219_CURRENT_LSB_VALUE    0.000001
...
int ina219_start(int *fd, int address) {
    ...
    if (i2c_dev_open(fd, devName, address) < 0) {
        printf("Pack Fail to initialize INA219 device at address: %x\n", address);
        return -1;
    }

    // Calibration I2C slave device
    if (ina219_calibrate(*fd, INA219_SHUNT_VALUE) < 0) {
        printf("Pack Fail to calibrate INA219 device\n");
        return -1;
    }

    // Configure I2C slave device
    if (ina219_configure(*fd) < 0) {
        printf("Pack Fail to configure INA219 device\n");
        return -1;
    }
...
ina219_data read_sensor_data(int i2c_slave_address) {
    ...
    // Read Current
    current = ina219_get_shunt_current(fd);
